...
void ITMVisualisationEngine_CPU::FindVisibleBlocks(const Scene* scene,
                                               ...
                                               ITMRenderState* renderState) const
{
	...
	for (int pointId = 0; pointId < noMissingPoints; pointId++)
	{
		...
		castRay<ITMVoxel, ITMVoxelIndex>(forwardProjection[locId], nullptr, nullptr, x, y, voxelData, voxelIndex, invM,
		                                 invProjParams,
...
#pragma omp parallel for
	...
	} else
	{
		for (int locId = 0; locId < imgSize.x * imgSize.y; ++locId)
		{
			...
			castRay<ITMVoxel, ITMVoxelIndex>(
				pointsRay[locId],
