...
ITMICPTracker_CPU::ITMICPTracker_CPU(Vector2i imgSize_d, Vector2i imgSize_rgb, const Parameters& parameters, const ITMLowLevelEngine *lowLevelEngine)
	...
	for (int y = 0; y < viewImageSize.y; y++) for (int x = 0; x < viewImageSize.x; x++)
	{
		...
		case TRACKER_ITERATION_ROTATION:
			isValidPoint = computePerPointGH_Depth<true, true>(localNabla, localHessian, localF, x, y, depth[x + y * viewImageSize.x], viewImageSize,
			                                                   viewIntrinsics, sceneImageSize, sceneIntrinsics, approxInvPose, renderedScenePose, pointsMap, normalsMap, distThresh[levelId]);
			break;
