...
template<typename T>
_CPU_AND_GPU_CODE_ inline Vector4f interpolateBilinear(const ORUtils::Vector4<T>* source,
...
template<typename T>
_CPU_AND_GPU_CODE_ inline float interpolateBilinear_single(const T* source,
                                                           const Vector2f& position, const Vector2i& imgSize)
{
	const Vector2i p((int) floor(position.x), (int) floor(position.y));
	const Vector2f delta(position.x - (float) p.x, position.y - (float) p.y);
...
template<typename T>
_CPU_AND_GPU_CODE_ inline Vector2f interpolateBilinear_Vector2(const ORUtils::Vector2<T>* source,
                                                               const Vector2f& position, const Vector2i& imgSize)
{
	const Vector2i p((int) floor(position.x), (int) floor(position.y));
	const Vector2f delta(position.x - (float) p.x, position.y - (float) p.y);
...
template<typename T>
_CPU_AND_GPU_CODE_ inline Vector4f interpolateBilinear_withHoles(const ORUtils::Vector4<T>* source,
                                                                 const Vector2f& position, const Vector2i& imgSize)
{
...
template<typename T>
_CPU_AND_GPU_CODE_ inline float interpolateBilinear_withHoles_single(const T* source,
                                                                     const Vector2f& position, const Vector2i& imgSize)
{
	const Vector2i p((int) floor(position.x), (int) floor(position.y));
	const Vector2f delta(position.x - (float) p.x, position.y - (float) p.y);
