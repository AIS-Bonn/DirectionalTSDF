...
_CPU_AND_GPU_CODE_ inline bool computePerPointGH_exDepth_Ab(float* A, float& b,
                                                            ...
                                                            const Vector4f* normalsMap, float spaceThresh,
...
_CPU_AND_GPU_CODE_ inline bool computePerPointGH_exRGB_inv_Ab(
	...
	const Vector2i& imgSize_depth, // and also size of intensities_curr due to the caching
...
)
{
	...
	if (pt_prev_proj.x < 0 || pt_prev_proj.x >= imgSize_rgb.x - 1 ||
	    pt_prev_proj.y < 0 || pt_prev_proj.y >= imgSize_rgb.y - 1)
		return false;
...
computePerPointGH_exDepth(float* localNabla, float* localHessian, float& localF, const int& x, const int& y,
                          const float& depth, float& depthWeight, Vector2i& viewImageSize,
                          const Vector4f& viewIntrinsics,
                          const Vector2i& sceneImageSize, const Vector4f& sceneIntrinsics,
                          const Matrix4f& approxInvPose, const Matrix4f& scenePose,
                          const Vector4f* pointsMap, const Vector4f* normalsMap, float spaceThresh,
                          float viewFrustum_min, float viewFrustum_max, float tukeyCutOff, int framesToSkip,
                          int framesToWeight)
{
	...
	bool ret = computePerPointGH_exDepth_Ab<shortIteration, rotationOnly, useWeights>(A, b, x, y, depth, depthWeight,
	                                                                                  viewImageSize, viewIntrinsics,
	                                                                                  sceneImageSize, sceneIntrinsics,
	                                                                                  approxInvPose, scenePose, pointsMap,
	                                                                                  normalsMap, spaceThresh,
	                                                                                  viewFrustum_min, viewFrustum_max,
	                                                                                  tukeyCutOff, framesToSkip,
	                                                                                  framesToWeight);
