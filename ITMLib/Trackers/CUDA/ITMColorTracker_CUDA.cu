...
__global__ void
colorTrackerOneLevel_f_device(Vector2f* out, Vector4f* locations, Vector4f* colours, Vector4u* rgb, int noTotalPoints,
                              Matrix4f M, Vector4f projParams, Vector2i imgSize);

__global__ void
colorTrackerOneLevel_g_rt_device(float* g_out, float* h_out, Vector4f* locations, Vector4f* colours, Vector4s* gx,
                                 Vector4s* gy, Vector4u* rgb,
                                 int noTotalPoints, Matrix4f M, Vector4f projParams, Vector2i imgSize);

__global__ void
colorTrackerOneLevel_g_ro_device(float* g_out, float* h_out, Vector4f* locations, Vector4f* colours, Vector4s* gx,
                                 Vector4s* gy, Vector4u* rgb,
                                 int noTotalPoints, Matrix4f M, Vector4f projParams, Vector2i imgSize);

// host methods

ITMColorTracker_CUDA::ITMColorTracker_CUDA(Vector2i imgSize, TrackerIterationType* trackingRegime,
                              ...
                              Matrix4f M, Vector4f projParams, Vector2i imgSize)
{
	...
	out_shared[locId_local].x = 0;
	out_shared[locId_local].y = 0;
	__syncthreads();
